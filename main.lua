...
-- Persistent UI Position for Rayfield
task.spawn(function()
    local CoreGui = game:GetService("CoreGui")
    local UIS = game:GetService("UserInputService")

    -- Wait for Rayfield to fully load
    local mainUI
    repeat
        task.wait()
        if CoreGui:FindFirstChild("Rayfield") then
            for _, gui in ipairs(CoreGui.Rayfield:GetDescendants()) do
    ...
    if isfile("2tuff_ui_pos.txt") then
        local pos = readfile("2tuff_ui_pos.txt")
        local success, posData = pcall(function() return loadstring("return " .. pos)() end)
        if success and typeof(posData) == "UDim2" then
            mainUI.Position = posData
        end
    end
        ...
        if input.UserInputType == Enum.UserInputType.MouseButton1 then
            ...
            input.Changed:Connect(function()
                if input.UserInputState == Enum.UserInputState.End then
                    dragging = false
                    writefile("2tuff_ui_pos.txt", tostring(mainUI.Position))
                end
            end)
        end
    end)
