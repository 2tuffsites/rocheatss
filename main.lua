...
local TpDropdown = TeleportsTab:CreateDropdown({
    ...
    Callback = function(option)
        ...
        -- Debugging part: Check if the location table contains the selected option
        print("Teleporting to: " .. option)

        if location[option] then
            local hrp = game.Players.LocalPlayer.Character and game.Players.LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
            if hrp then
                hrp.CFrame = location[option]
                print("Teleported successfully to " .. option)
            else
                print("Failed to find HumanoidRootPart")
            end
        else
            print("Location not found: " .. option)
        end
    end
})
